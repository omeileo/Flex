...
import { ResendVerifyEmailRequest, VerifyEmailWithCodeRequest } from './verifyEmail.types'
...
export const verifyEmail = createAsyncThunk(
  ...
  async (request: VerifyEmailWithCodeRequest, { rejectWithValue }) => {
    try {
      ...
      const serialized = serializeError(error) as { userFriendlyMessage?: string, message?: string }

      return rejectWithValue({
        message: serialized.userFriendlyMessage ?? serialized.message ?? 'Verification failed'
      })
    }
  }
)
...
export const resendVerifyEmail = createAsyncThunk(
  ...
  async (request: ResendVerifyEmailRequest, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue({
        message: serialized.userFriendlyMessage ?? serialized.message ?? 'Could not resend code'
      })
    }
  }
)
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(verifyEmail.rejected, (state, { payload }) => {
        state.loading = false
        state.error = (payload as { message?: string })?.message ?? 'Verification failed'
      })
      ...
      .addCase(resendVerifyEmail.rejected, (state, { payload }) => {
        state.resendLoading = false
        state.error = (payload as { message?: string })?.message ?? 'Could not resend code'
      })
  }
})

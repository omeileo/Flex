...
const WorkoutSessionContainer = () => {
  ...
  const { success: plan } = useSelector(
    (state: RootState) => state.getActivePlan,
  );
  const { loading: creating, error: createError } = useSelector(
    (state: RootState) => state.createWorkoutSession,
  );
  const { loading: completing, error: completeError } = useSelector(
    (state: RootState) => state.completeWorkoutSession,
  );
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      const created = await dispatch(
        createWorkoutSession(sessionPayload),
      ).unwrap();

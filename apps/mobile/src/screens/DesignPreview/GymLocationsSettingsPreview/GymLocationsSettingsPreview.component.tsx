...
import {
  GymLocationsSettingsPreviewComponentProps,
  GymLocationsView,
} from './GymLocationsSettingsPreview.types';
...
const GymLocationsSettingsPreviewComponent = (
  ...
  const renderList = () => (
    <>
      <Text style={styles.title}>
        {t('designPreview.gymLocations.listTitle')}
      </Text>
      <Text style={styles.subtitle}>
        {t('designPreview.gymLocations.listSubtitle')}
      </Text>
      {locations.map(loc => (
        <LocationCard
          key={loc.id}
          ...
          presetType={loc.presetType}
          equipmentCount={
            loc.equipment.length || (loc.id === 'loc-home' ? 8 : 24)
          }
          isDefault={loc.isDefault}
      ...
      <Text style={styles.sectionLabel}>
        {t('designPreview.gymLocations.addLocationLabel')}
      </Text>
      <TextInput
        style={styles.input}
    ...
    <>
      ...
      <Pressable style={styles.row} onPress={() => setView('equipment')}>
        <Text style={styles.rowLabel}>
          {t('designPreview.gymLocations.equipmentRow')}
        </Text>
      ...
      <Pressable style={styles.row} onPress={() => setView('list')}>
        <Text style={styles.rowLabel}>
          {t('designPreview.gymLocations.setDefault')}
        </Text>
        <Text style={styles.rowValue}>
      ...
      <PrimaryButton
        label={t('designPreview.gymLocations.backToList')}
  ...
  const renderEquipment = () => (
    <>
      <Text style={styles.title}>
        {t('designPreview.gymLocations.equipmentTitle')}
      </Text>
      ...
      />
      <PrimaryButton
        label={t('designPreview.gymLocations.saveEquipment')}
        ...
        <View style={styles.viewChips}>
          {previewChips.map(chip => (
            <Pressable
              ...
              style={[
                styles.viewChip,
                view === chip.key && styles.viewChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.viewChipText,
                  view === chip.key && styles.viewChipTextActive,
                ]}
              >
                {chip.label}
              </Text>

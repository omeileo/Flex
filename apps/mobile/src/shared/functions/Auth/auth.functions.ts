...
export const isAccountNotVerifiedError = (error: {
  ...
  if (error.details?.some((detail) => detail.issue === 'not_verified')) {
    return true
  }

  return (error.message ?? '').toLowerCase().includes('not been verified')
}

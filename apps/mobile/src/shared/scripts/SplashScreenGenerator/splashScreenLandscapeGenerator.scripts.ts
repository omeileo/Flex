...
;(async function generateLandscapeImage() {
  try {
    ...
    console.log('\033[1m', 'Generating landscape splash image...', '\x1b[0m')
    ...
    const defaultCenteredYOffset = Math.floor(portraitImageHeight / 2 - 805 / 2)
    const yOffset = isNaN(yOffsetArgument) ? defaultCenteredYOffset : yOffsetArgument
    const xOffset = 0
    const cropToHeight = 805

    const cropped = portraitImage.crop({ x: xOffset, y: yOffset, w: portraitImageWidth, h: cropToHeight })

    await cropped.write(outputFilename)

    console.log('\x1b[32m', 'Landscape splash created.', '\x1b[0m')
  } catch (error) {
    console.log('ERROR: \x1b[31m', 'Failed to create landscape splash.', '\x1b[0m')
    throw error
  }
})()

...
function generateSplashForIos() {
  console.log(
    '\x1b[1m',
    'Generating splash image for iOS devices...',
    '\x1b[0m',
  );
  ...
  fs.copyFile(imageSourcePath, iosDestinationPath, error => {
    if (error) {
      console.log(
        'ERROR: \x1b[31m',
        'Failed to generate splash image for iOS devices.',
        '\x1b[0m',
        error.message,
      );
    } else {
      console.log(
        '\x1b[32m',
        'Splash image generated for iOS devices.',
        '\x1b[0m',
      );
    }
  });
...
async function generateSplashForAndroid() {
  console.log(
    '\x1b[1m',
    'Generating splash images for Android devices...',
    '\x1b[0m',
  );
  ...
  for (const item of androidResolutions) {
    const outputPath =
      androidDestinationMain +
      `${androidDestinationSubFolders[item.resolution]}/`;
    const outputFilename = 'launch_screen.jpg';

    try {
      const image = await Jimp.read(imageSourcePath);
      const scaledPortraitImage = image.resize({
        w: item.width,
        h: item.height,
      });
      await scaledPortraitImage.write(outputPath + outputFilename, {
        quality: 90,
      });

      console.log(
        '\x1b[32m',
        `Launch screen created for resolution: ${item.resolution}.`,
        '\x1b[0m',
      );
    } catch (error) {
      console.log(
        'ERROR: \x1b[31m',
        `Failed to create launch screen for resolution: ${item.resolution}.`,
        '\x1b[0m',
      );
      throw error;
    }

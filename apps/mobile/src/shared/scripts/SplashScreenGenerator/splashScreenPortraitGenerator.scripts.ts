...
const landscapeImageSourcePath =
  'src/assets/images/splash_screen_landscape.png';

(async function generatePortraitImage() {
  try {
    if (!fs.existsSync(landscapeImageSourcePath)) {
      throw new Error(
        `Source image ${landscapeImageSourcePath} was not found.`,
      );
    }

    console.log('\x1b[1m', 'Generating portrait splash image...', '\x1b[0m');
    ...
    const defaultCenteredXOffset = Math.floor(
      landscapeImageWidth / 2 - 805 / 2,
    );
    const xOffset = isNaN(xOffsetArgument)
      ? defaultCenteredXOffset
      : xOffsetArgument;
    const yOffset = 0;
    const cropToWidth = 805;

    const cropped = landscapeImage.crop({
      x: xOffset,
      y: yOffset,
      w: cropToWidth,
      h: landscapeImageHeight,
    });
    ...
    console.log(
      'ERROR: \x1b[31m',
      'Failed to create portrait splash.',
      '\x1b[0m',
    );
    throw error;
  }

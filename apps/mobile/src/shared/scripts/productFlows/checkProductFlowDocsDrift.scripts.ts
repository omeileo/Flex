...
function collectFlowKeysFromPath(
  relativePath: string,
  into: Set<FlowKey>,
): void {
  const normalized = normalizePath(relativePath);
...
const SUBSTANCE_EXEMPT_DOC_PATHS = new Set([
  'docs/productFlows.overview.md',
  'docs/productFlows.changelog.md',
]);

function isSubstantiveDocChange(
  docPath: string,
  ...
  const onlyLastReviewedBumps = changedLines.every(line =>
    /^[+-]\s*lastReviewed\s*:/.test(line),
  );
...
type CoverageResult =
  | { covered: true }
  | { covered: false; reason: 'missing' | 'trivial'; flowKey: FlowKey };
...
function docsCoverFlowKeys(
  ...
  for (const flowKey of requiredKeys) {
    ...
    const hasSubstantive = candidatePaths.some(p =>
      isSubstantiveDocChange(p, mode),
    );
    if (!hasSubstantive) {
...
function main(): void {
  ...
  const coveragePaths =
    mode === 'workingTree' ? triggerPaths : listGitPaths('staged');

...
export const getFlexApiToken = async (): Promise<string | null> => {
  try {
    ...
    logger.logError(error, 'Failed to read auth token from keychain', 'getFlexApiToken')
...
export const setFlexApiToken = async (token: string): Promise<void> => {
  try {
    await Keychain.setGenericPassword('flexUser', token, { service: TOKEN_SERVICE })
  } catch (error) {
    logger.logError(error, 'Failed to store auth token in keychain', 'setFlexApiToken')
    throw error
  }
}
...
export const clearFlexApiToken = async (): Promise<void> => {
  try {
    ...
    logger.logError(error, 'Failed to clear auth token from keychain', 'clearFlexApiToken')
  }
}

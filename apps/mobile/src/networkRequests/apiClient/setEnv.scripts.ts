...
if (!targetEnv) {
  console.error(
    'Please provide the target environment as a parameter (e.g., development, staging, production, local).',
  );
  process.exit(1);
}
...
if (!validEnvironments.includes(targetEnv)) {
  console.error(
    `Invalid environment. Provide one of: ${validEnvironments.join(', ')}.`,
  );
  process.exit(1);
}
...
const envDirectory = path.resolve(
  process.cwd(),
  'src/networkRequests/environmentVariables',
);
...
try {
  ...
  console.error(
    `Error copying env file: ${error instanceof Error ? error.message : error}`,
  );
  process.exit(1);
}

...
export default function date(dateTime?: Date | string | number, dateFormat?: string) {
  ...
  const defaultDate: DateTime<boolean> = getValidatedDateTime(
  ...
  return {
     ...
     */
    difference: (dateTime: DateTime | string | number, dateFormat?: string, duration?: DateDurationUnit): number => {
     ...
     */
    removeTimeFromISOString: (isoDateTimeString: string): string => {
    ...
    /**
     * Calculates the duration between two dates in milliseconds and returns the ISO 8601 duration string.
     * @param {string} startDate - The start date.
     * @param {string} endDate - The end date.
     * @returns {string} The ISO 8601 duration string.
     * @example date().calculatedDuration('2024-01-01', '2024-01-02') // returns 'PT24H'
     */
    calculatedDuration: (startDate: string, endDate: string): string => {
      const duration = date(endDate).difference(startDate)
      const isoDuration = date().convertMillisecondsToISO8601(duration)
    ...
    parseISO8601Duration: (isoDuration: string): Duration | null => {
      try {
        ...
        logger.error('Error parsing ISO 8601 duration string', error)
...
const getValidatedDateTime = (dateTime: DateTime | string | number, dateFormat?: string) => {
  let defaultDate: DateTime<boolean> = DateTime.fromObject({
    year: 0,
    month: 0,
    day: 0
  })
